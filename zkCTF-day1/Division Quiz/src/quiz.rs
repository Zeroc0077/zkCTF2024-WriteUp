    ...
    pub fn assign_witness(
        ...
        let (c_cell, _) = layouter.assign_region(
            ...
            |mut region| {
                ...
                let a_cell = region.assign_advice(|| "a", config.a, 0, || Value::known(a))?;
                let b_cell = region.assign_advice(|| "b", config.b, 0, || Value::known(b))?;
                let c_cell = region.assign_advice(|| "c", config.c, 0, || Value::known(c))?;
